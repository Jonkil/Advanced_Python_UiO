...
def extract_events(table: bs4.element.Tag) -> pd.DataFrame:
    ...
    for tr in rows:
        ...
        row = []
        for cell in cells:
            cs = re.search(r'(colspan="\d{1,2}")', str(cell))
            colspan = 1
            rowspan = 1
            if cs:
                colspan = int(cs.group()[-2:-1])
            rs = re.search(r'(rowspan="\d{1,2}")', str(cell))
            if rs:
                rowspan = int(rs.group()[-2:-1])
...
def filter_data(keys: list, data: list, wanted: list):
    ...
    """
    idxs = [keys.index(col) for col in wanted]
    min_n_cols = max(idxs)
    # some rows in the table may be merged and we get
    # less number of columns than needed
    # we omit such rows
    filtered_data = [[l[i] for i in idxs if len(l)>=min_n_cols] for l in data]
